...
if __name__ == '__main__':
    ...
    # Get list of charging pad names from launch file
    ...
    if qualisys_connected:
        ...
        for pad in pad_names:
            ...
            charger_pos_subscribers.append(rospy.Subscriber("/charger_pos" + str(pad[-1]), PointStamped,
                                                            charger_callback, pad_names.index(pad)))

            # Check for successful subscription to charger position topic(s)
            try:
                rospy.wait_for_message("/charger_pos" + str(pad[-1]), PointStamped, timeout=1)
            except rospy.ROSException:
    ...
    for crazy_inst in crazy_instances:
        ...
        t.append(Thread(target=flight_commands, args=(crazy_inst, num_chargers, bt), daemon=True))
