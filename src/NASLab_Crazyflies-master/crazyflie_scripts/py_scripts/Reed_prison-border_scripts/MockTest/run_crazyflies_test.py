...
import flight_commands_test
from flight_commands_test import testing, track_object
...
if __name__ == '__main__':
    ...
    if not group_1[1]:
        print('hey')
        for i in range(0,len(group_2[0])-1):
            t.append(Thread(target=testing, args=(group_2[0][i][0], group_2[1][i][0],bt,lambda: cf_spotted,lambda: stop_threads),daemon=True))
            t.append(Thread(target=testing, args=(group_2[0][i][1], group_2[1][i][1],bt,lambda: cf_spotted,lambda: stop_threads),daemon=True))
            #circle_2(group_2[0][i][0],group_2[0][i][1],cent_pos[i][0],cent_pos[i][1],0.5)
    else:
        print('hey2')
        t.append(Thread(target=testing, args=(group_1[0][0],group_1[1][0],bt,lambda: cf_spotted,lambda: stop_threads),daemon=True))
        #circle_1(group_1[0][0],cent_pos[0][0],cent_pos[0][1],0.5)
        for i in range(0,len(group_2[0])-1):
            t.append(Thread(target=testing, args=(group_2[0][i][0],group_2[1][i][0],bt,lambda: cf_spotted,lambda: stop_threads),daemon=True))
            t.append(Thread(target=testing, args=(group_2[0][i][1],group_2[1][i][0],bt,lambda: cf_spotted,lambda: stop_threads), daemon=True))
            #circle_2(group_2[0][i][0],group_2[0][i][1],cent_pos[i+1][0],cent_pos[i+1][1],[1],0.5)

    t.append(Thread(target=testing, args=(job3[0][0], job3[1][0], bt, lambda: cf_spotted, False),daemon=True))
        ...
        if flag2 == 0: # used to run nominal mission (I.E. takeoff, begin pattern) only once
            ...
            if spotted:  # condition to say script should move on from nominal
                ...
                if flag == 1: # if actual spotting made, assign camera drone to stay in the air
                    ...
                    #Check to see if net laucher is under camera drone, if it is, choose different net #########################################################################
